...
public class BlockOreDropOLD extends BlockBase {

    private final Item dropItem;
    private final int baseDropCount;
    private final int harvestLevel;

    public BlockOreDropOLD(String name, Item dropItem, int baseDropCount, int harvestLevel) {
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        ...
        if (!worldIn.isRemote) {
            EntityPlayer player = worldIn.getClosestPlayer(pos.getX(), pos.getY(), pos.getZ(), Double.MAX_VALUE, false);
            if (player != null && !player.isCreative()) {
                ItemStack tool = player.getHeldItemMainhand();
                int toolHarvestLevel = tool.getItem().getHarvestLevel(tool, "pickaxe", player, state);

                if (toolHarvestLevel >= this.harvestLevel) {
                    ...
                    totalDrops = Math.max(totalDrops, fortuneLevel);
    ...
    private int calculateTotalDrops(int baseDropCount, int fortuneLevel) {
        ...
        int extraDrops = fortuneLevel > 0 ? random.nextInt(fortuneLevel + 1) : 0;
